...
export const SystemHealth = () => {
    ...
    const solutionItem = (labelKey, level, key) => {
        const ok = level !== 'Low';
        const displayLevel = level === 'OK' ? t('ok') : t('low');
        return (
            <div
                key={key}
        ...
        <div className="rounded-lg border border-border bg-card p-5" data-testid="system-health-panel">
            <div className="mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-display text-base font-semibold">{t('systemHealth')}</h3>
                </div>
                <span className="inline-flex items-center gap-1 rounded-full border border-[hsl(var(--ph-optimal)/0.4)] bg-[hsl(var(--ph-optimal)/0.10)] px-2 py-0.5 font-mono text-[10px] uppercase tracking-widest text-[hsl(var(--ph-optimal))]">
                    <CheckCircle2 className="h-3 w-3" /> {t('nominal')}
                </span>
            </div>
            ...
            <div className="divide-y divide-border">
                <Row
                    ...
                    icon={Cpu}
                    label={t('soilSensors')}
                    value={`${health.sensors.online}/${health.sensors.count} ${t('pumpsOnline')}`}
                ...
                <Row
                    ...
                    icon={Activity}
                    label={t('pumps')}
                    value={`${health.pumps.online}/${health.pumps.total} ${t('pumpsReady')}`}
            ...
            {health.solutions.base === 'Low' && (
                ...
                >
                    ...
                    <span>{t('baseSolutionWarning')}</span>
                </div>
            )}
        </div>
    );
