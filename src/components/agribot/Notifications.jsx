...
export const Notifications = () => {
    ...
    const formatRel = (time) => {
        const diff = Math.floor((Date.now() - time) / 60_000);
        if (diff < 1) return t('justNow');
        if (diff < 60) return `${diff}${t('mAgo')}`;
        return `${Math.floor(diff / 60)}${t('hAgo')}`;
    };
        ...
        <div className="rounded-lg border border-border bg-card p-5" data-testid="notifications-panel">
            <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-display text-base font-semibold">{t('smartAlerts')}</h3>
            ...
            {notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 rounded-md border border-dashed border-border p-8 text-center">
                    ...
                    <p className="font-display text-sm font-medium">{t('noAlerts')}</p>
                    <p className="text-xs text-muted-foreground">{t('noActiveAlerts')}</p>
                </div>
            ) : (
                <ScrollArea className="h-[280px] pr-2">
                    <ul className="space-y-2">
                        {notifications.map((n) => {
                            ...
                            const displayBody = n.bodyKey ? t(n.bodyKey) : n.body;
                            // Countdown logic
                            let countdown = null;
                            if (n.autoApproveAt) {
                                const ms = n.autoApproveAt - Date.now();
                                if (ms > 0) {
                                    const min = Math.floor(ms / 60000);
                                    const sec = Math.floor((ms % 60000) / 1000);
                                    countdown = `${min}:${sec.toString().padStart(2, '0')} ${t('untilAutoApprove')}`;
                                } else {
                                    countdown = t('autoApproving');
                                }
                            }
                            return (
                                <li
                                ...
                                >
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="font-display text-sm font-semibold text-foreground">{displayTitle}</p>
                                            <p className="mt-0.5 text-xs text-muted-foreground">{displayBody}</p>
                                            <p className="mt-1 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                                                {formatRel(n.time)}
                                            </p>
                                            {countdown && (
                                                <p className="mt-0.5 text-xs text-orange-600 font-mono">{countdown}</p>
                                            )}
                                        </div>
                                    </div>
                                    {n.type !== 'success' && (
